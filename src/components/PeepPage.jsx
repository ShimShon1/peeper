...
export default function PeepPage({
  ...
  async function deleteComment(content) {
    await updateDoc(peepRef, {
      comments: peep.comments.filter((comment) =>
        content === comment.content && comment.userPosted.uid === user.uid
          ? false
          : true
      ),
    });
  ...
  return (
    <>
      ...
      <div className="ml-[72px] text-sm space-x-4 px-4 lg:px-8 pb-2">
        <span>
          {" "}
          <span className="font-semibold">{peep.comments.length}</span>{" "}
          {peep.comments.length > 1 ? "Comments" : "Comment"}
        </span>
        <span>
          {" "}
          <span className="font-semibold">{peep.likedBy.length}</span>{" "}
          {peep.likedBy.length > 1 ? "Likes" : "Like"}
        </span>
      </div>
      ...
      <div className="flex ml-[72px] p-4  justify-around w-3/4 ">
        ...
        <img
          src={liked ? likeIconGreen : likeIcon}
          onClick={() => likePeep(peep)}
          alt=""
        />
        {peep.userPosted.uid === user.uid ? (
          <Link to={"/"}>
            {" "}
            <img src={trashIcon} onClick={() => deletePeep(peep)} alt="" />
          </Link>
        ) : (
      ...
      <form
        className="w-full flex p-4 lg:p-8 border-t pb-2 lg:pb-2"
      ...
      >
        ...
        <div className="pt-3 flex-grow text-xl">
          <textarea
            ...
            value={newComment}
            className="resize-none indent-2 relative  w-full  focus:outline-none"
            placeholder={user ? "Peep your reply" : "Log in to reply!"}
          />
          <div className="flex items-center justify-end ">
            <div>{user && <SubmitBtn content={"Reply"} />}</div>
          </div>
        </div>
      </form>
      <hr />
