...
export default function Title({ path }) {
  ...
  return (
    <div
      className="col-span-9 bg-slate-50  w-full p-4 grid 
        grid-cols-9 sticky top-0 z-10 bg-opacity-40 items-center 
        lg:px-8 "
    >
      <div className="lg:col-span-1 col-span-2  flex xl:col-start-2 ">
        <Link to={"/"}>
          <img src={Logo} className="w-14" alt="home Icon" />
        </Link>
      </div>

      <div className="col-span-5 lg:col-start-3 text-2xl font-semibold">
        {page}
      </div>
    </div>
  );
